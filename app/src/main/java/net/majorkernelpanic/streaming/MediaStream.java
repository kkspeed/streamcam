...
public abstract class MediaStream implements Stream {
	...
	protected void createSockets() throws IOException {
			HandlerThread thread = new HandlerThread("PipeHandler");
			thread.start();
			Handler handler = new Handler(thread.getLooper());

			StorageManager sm = H264VideoStream2.sContext.getSystemService(StorageManager.class);

			ParcelFileDescriptor pf = sm.openProxyFileDescriptor(
					...
					new ProxyFileDescriptorCallback() {
						...
						public int onRead(long offset, int size, byte[] data) throws ErrnoException {
						  Log.e(TAG, "OnRead " + offset + " " + size);
							int dest = (int) Math.min(offset + size, end);
						  for (int i = (int) offset; i < dest; i++) {
								data[i - (int)offset] = buffer[i];
							}
						  return dest - (int) offset;
						}

						@Override
						public int onWrite(long offset, int size, byte[] data) throws ErrnoException {
							Log.e(TAG, "OnWrite" + offset + " " + size);
							for (int i = (int)offset; i < size; i++) {
								buffer[i] = data[i - (int) offset];
							}
							if ((int) offset + size > end) {
								end = (int) offset + size;
							}
							return size;
						}
			...
			mParcelRead = new ParcelFileDescriptor(pf); // new ParcelFileDescriptor(mParcelFileDescriptors[0]);
			mParcelWrite = new ParcelFileDescriptor(pf); // new ParcelFileDescriptor(mParcelFileDescriptors[1]);
	}
